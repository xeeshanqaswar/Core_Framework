...
{
    class LeaderboardsDeploymentHandler : ILeaderboardsDeploymentHandler
    {
        readonly ILeaderboardsClient m_Client;
        readonly object m_ResultLock = new();
        const int k_BatchSize = 9; // Rate limit is 10 per second, but we call the List api before deploying
            ...
            CancellationToken token = default)
        {
            ...
            await Batching.Batching.ExecuteInBatchesAsync(allTasks, token, k_BatchSize);
